...
function DropdownLoginUser(props: TypeProps) {
  const { Image } = props;
  const ImageError =
      ...
      <ul className={`dropdown-menu`} aria-labelledby="dropdownMenuuser">
        ...
        <li>
          <Link href={`/myprofile/my-study-requests/current/${1}`}>
            {i18n?.t("common:RequestsToCreateAStudy")}
          </Link>
        </li>
        <li>
        ...
        <li>
          <Link href={`/myprofile/my-studies-uploaded/${1}`}>
            {i18n?.t("common:PurchasedStudies")}
          </Link>
        </li>
        <li>
...
export default DropdownLoginUser;
